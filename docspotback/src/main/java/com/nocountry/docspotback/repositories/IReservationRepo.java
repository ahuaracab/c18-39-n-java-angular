...
public interface IReservationRepo extends IGenericRepo<Reservation, UUID> {
    ...
    @Query(value = "SELECT r.*, p.id as patien_id , p.name_patient, p.photo_patient, p.cellphone_patient\n" +
            "FROM reservations r\n" +
    ...
    List<Reservation> findByProfessional(@Param("professionalId") UUID professionalId);
}
