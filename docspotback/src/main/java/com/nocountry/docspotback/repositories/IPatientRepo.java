...
public interface IPatientRepo extends IGenericRepo<Patient, UUID>  {
    ...
    @Query(value = "SELECT pa.id_patient,pa.name_patient,pa.cellphone_patient,pa.photo_patient,pa.has_social_work,pa.social_work,pa.created_at,pa.updated_at,pa.deleted_at,pa.is_deleted,pa.user_id_user FROM patients pa INNER JOIN users us ON us.id_user=pa.user_id_user WHERE us.id_user=:userId",nativeQuery = true)
