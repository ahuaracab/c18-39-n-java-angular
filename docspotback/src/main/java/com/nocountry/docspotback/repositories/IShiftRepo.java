...
public interface IShiftRepo extends IGenericRepo<Shift, UUID>{
	...
	@Query(value = "FROM shifts s INNER JOIN professionals_shifts ps ON ps.id_shift=s.id_shift WHERE s.state_shift=true and ps.id_professional=:idProfessional",nativeQuery = true)
	List<Shift> availableShiftsByProfessional(@Param("idProfessional") UUID idProfessional);
}
