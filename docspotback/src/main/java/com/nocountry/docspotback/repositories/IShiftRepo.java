...
public interface IShiftRepo extends IGenericRepo<Shift, UUID>{
	...
	@Query(value = "SELECT s.id_shift,s.date_shift,s.hours_time,s.repeat_shift,s.state_shift,s.created_at,s.deleted_at,s.updated_At,s.is_deleted FROM shifts s \r\n"
			...
			+ "	WHERE (s.state_shift=true and ps.id_professional=:idProfessional and s.date_shift=:dateShift and s.date_shift>current_date) or (s.state_shift=true and ps.id_professional=:idProfessional and s.date_shift=:dateShift and s.date_shift=current_date and s.hours_time>current_time)\r\n"
